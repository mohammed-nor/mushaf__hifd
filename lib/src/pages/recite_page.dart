  ...
  Future<void> _generateRandomThomun() async {
    ...
    if (_onlyLearned) {
      ...
      if (learnedList.isNotEmpty) {
        ...
        final filtered = learnedList.map(int.tryParse).whereType<int>().where((i) => i >= 0 && i < kThomunsTxt.length).toList();
    ...
    try {
      final text = await rootBundle.loadString('lib/thomuns_txt/${kThomunsTxt[selectedIndex]}');
      if (mounted) {
        setState(() {
                    ...
                    child: _currentThomunIndex != null && _thomunText != null
                              ...
                              child: Padding(
                                ...
                                child: SingleChildScrollView(
                                  child: Text(
                                    ...
                                    style: Theme.of(context).textTheme.titleLarge!.copyWith(height: 1.9, fontWeight: FontWeight.normal),
                                  ),
                                ),
                              ),
                            ),
                        ...
                        : Center(
                            child: Column(
                              ...
                              children: [
                                Icon(Icons.auto_stories, size: 80, color: Colors.white.withAlpha(26)),
                                const SizedBox(height: 16),
                                Text(
                                  'لم يتم اختيار ثمن بعد',
                                  style: Theme.of(context).textTheme.bodyLarge!.copyWith(color: Colors.white.withAlpha(128), height: settings.lineSpacing),
                                ),
                              ],
                            ),
                          ),
